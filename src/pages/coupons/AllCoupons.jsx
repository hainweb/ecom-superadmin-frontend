...
const AllCoupons = () => {
  ...
  useEffect(() => {
    const fetchCoupons = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/get-all-coupons`, {
  ...
  const handleDeleteCoupon = async (couponCode) => {
    ...
    try {
      const response = await axios.delete(
        `${BASE_URL}/delete-coupon/${couponCode}`
      );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Manage all users discount coupons
          </p>
        </div>
        ...
        {filteredCoupons.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCoupons.map((coupon) => (
              <div
                key={coupon._id}
                className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 dark:border-gray-700 ${getCardOpacity(
                  coupon
                )} hover:scale-105`}
              >
                {/* Status Badge */}
                ...
                {/* Coupon Content */}
                <div className="p-6 relative bg-white dark:bg-gray-800 rounded-lg shadow-md">
                  {/* 🔴 Delete Icon */}
                  <button
                    onClick={() => handleDeleteCoupon(coupon.code)}
                    className="absolute top-4 right-4 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-600 transition"
                    title="Delete Coupon"
                  >
                  ...
                  <div className="mt-4 text-center">
                    <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center justify-center gap-1">
                      ...
                      {isExpired(coupon.expDate) ? "Expired on" : "Valid until"}{" "}
                      {new Date(coupon.expDate).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                      })}
