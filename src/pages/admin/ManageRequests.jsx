...
import { CheckCircle, XCircle, Store, Mail, Phone, Calendar, FileText, Building } from "lucide-react";
...
const ManageRequests = () => {
  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      ...
      await axios.post(`${BASE_URL}/approve-merchant`, {
        merchantId: selectedMerchant._id
      });
      
      // Remove approved merchant from list
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Merchant Approval Requests</h1>
          <p className="text-gray-600">Review and approve pending merchant registrations</p>
        </div>
        ...
        {merchants.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500">All merchant requests have been processed</p>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {merchants.map((merchant) => (
              <div key={merchant._id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-200">
                <div className="p-6">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to approve <strong>{selectedMerchant.Name}</strong>
              {selectedMerchant.BusinessName && ` (${selectedMerchant.BusinessName})`}? 
